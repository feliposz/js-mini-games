function initDemo() {
    ...
    loadImage("mechanic_spanner_base.png", function (e, d) {
        done("objTexture", e, d);
    });
    loadJSONResource("mechanic_spanner_base.json", function (e, d) {
        done("objModel", e, d);
    });
    loadTextResource("demo020-vertex.glsl", function (e, d) {
        done("vertexShaderSource", e, d);
    });
    loadTextResource("demo020-fragment.glsl", function (e, d) {
...
function runDemo(objModel, objTexture, vertexShaderSource, fragmentShaderSource) {
    ...
    var normals = new Float32Array(objModel.meshes[0].normals);
    ...
    shaderProgram = initProgram(gl, vertexShader, fragmentShader);

    gl.useProgram(shaderProgram);

    var posBuffer = initVertexBuffer(gl, gl.getAttribLocation(shaderProgram, "aPosition"), vertices, 3, 3, 0);
    var normalBuffer = initVertexBuffer(gl, gl.getAttribLocation(shaderProgram, "aNormal"), normals, 3, 3, 0);
    ...
    mat4.scale(mModel, mModel, [50, 50, 50]);
    mat4.lookAt(mView, [0, 0, 5], [0, 0, 0], [0, 1, 0]);
    mat4.perspective(mProj, Math.PI / 2, gl.drawingBufferWidth / gl.drawingBufferHeight, 0.1, 1000);
    //function ortho(out, left, right, bottom, top, near, far) {
    //mat4.ortho(mProj, -10, 10, -10, 10, 0.1, 1000);
    ...
    var ambientLightUniform = gl.getUniformLocation(shaderProgram, "ambientLight");
    var diffuseLightColorUniform = gl.getUniformLocation(shaderProgram, "diffuseLightColor");
    var diffuseLightDirectionUniform = gl.getUniformLocation(shaderProgram, "diffuseLightDirection");
    gl.uniform3f(ambientLightUniform, 0.2, 0.2, 0.5);
    gl.uniform3f(diffuseLightColorUniform, 0.9, 0.7, 0.4);
    gl.uniform3f(diffuseLightDirectionUniform, 0.0, 1.0, 1.0);
    ...
    function loop() {
        ...
        mat4.rotateX(mModel, mModel, Math.sin(t * .001) * 0.04);
        mat4.rotateY(mModel, mModel, Math.cos(t * .002) * 0.03);
